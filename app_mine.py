...
st.set_page_config(layout="wide", page_title="South Africa Water Quality & Mine Map")
st.title("📍 South Africa Water Quality & Mine Network")
...
show_mine = st.sidebar.checkbox("Show Mines (Green)", value=True)
...
radius_km = st.sidebar.selectbox(
    ...
    options=[0, 1, 3, 5, 10, 20, 30, 40], # [UPDATED] 40km 옵션 추가
    format_func=lambda x: "None" if x == 0 else f"{x} km",
    index=0
)

# 4. Map Setup
data_frames = [df for df in [df_train, df_val, df_mine] if not df.empty]
...
if show_mine:
    # Mine 마커 크기 1.25, 반경 기능 제외
    add_markers_and_radius(df_mine, 'lime', '#39FF14', 'Mine Facility', desc_col='Description', marker_size=1.25, with_radius=False)
...
mine_count = len(df_mine) if not df_mine.empty else 0
st.info(f"Loaded {river_count} river reaches. | Loaded {mine_count} mine sites.")